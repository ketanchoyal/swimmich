...
final class UploadViewModelLiveActivityTests: XCTestCase {
    ...
    func test_liveActivity_startThenUpdateThenEndOnDone() async {
        ...
        XCTAssertEqual(activity.startCalls[0].total, 0, "total inconnu avant dédup (pipeline streamé)")
        XCTAssertEqual(activity.updateCalls.count, 3, "une mise à jour par upload accepté (streamé : total croît)")
        XCTAssertEqual(activity.updateCalls[0].total, 1, "total découvre au fur et à mesure des chunks acceptés")
        XCTAssertEqual(activity.updateCalls[0].uploaded, 1)
        XCTAssertEqual(activity.updateCalls[2].uploaded, 3)
        XCTAssertEqual(activity.updateCalls[2].total, 3)
