...
final class SharedLinksViewModelTests: XCTestCase {
    ...
    private func makePartner(id: String, inTimeline: Bool = false) -> PartnerResponseDto {
        PartnerResponseDto(
    ...
    func test_partner_load_mapsList() async {
        let mock = MockImmichClient()
        mock.partnersResponse = [makePartner(id: "p1", inTimeline: true), makePartner(id: "p2")]
        let vm = SharedLinksViewModel(client: mock)
        await vm.loadPartners()
        XCTAssertEqual(vm.partners.map(\.id), ["p1", "p2"])
        XCTAssertTrue(vm.partners[0].inTimeline)
        XCTAssertFalse(vm.isPartnersLoading)
        XCTAssertNil(vm.partnersError)
    }

    func test_partner_load_failure_setsError() async {
        let mock = MockImmichClient()
        mock.partnersError = Boom()
        let vm = SharedLinksViewModel(client: mock)
        await vm.loadPartners()
        XCTAssertTrue(vm.partners.isEmpty)
        XCTAssertNotNil(vm.partnersError)
    }

    func test_partner_toggle_sendsUpdateAndReplacesRow() async {
        let mock = MockImmichClient()
        mock.partnersResponse = [makePartner(id: "p1")]
        let vm = SharedLinksViewModel(client: mock)
        await vm.loadPartners()

        await vm.togglePartnerTimeline(id: "p1", enabled: true)

        XCTAssertEqual(mock.lastUpdatePartnerId, "p1")
        XCTAssertEqual(mock.lastUpdatePartnerInTimeline, true)
        XCTAssertEqual(vm.partners[0].inTimeline, true, "row replaced with server response")
        XCTAssertNil(vm.partnersError)
    }

    func test_partner_toggle_failure_keepsRow() async {
        let mock = MockImmichClient()
        mock.partnersResponse = [makePartner(id: "p1")]
        let vm = SharedLinksViewModel(client: mock)
        await vm.loadPartners()
        ...
        await vm.togglePartnerTimeline(id: "p1", enabled: false)

        XCTAssertEqual(vm.partners[0].inTimeline, false, "failed toggle must not flip the row")
        XCTAssertNotNil(vm.partnersError)
    }

    func test_partner_remove_callsAndRemoves() async {
        let mock = MockImmichClient()
        mock.partnersResponse = [makePartner(id: "p1"), makePartner(id: "p2")]
        let vm = SharedLinksViewModel(client: mock)
        await vm.loadPartners()

        await vm.removePartner(id: "p1")
