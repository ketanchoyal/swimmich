...
`SearchFilterSheet` sans logique. Les
...
(retiré le 2026-09-15, cf. la note datée) ;
...
Note (décision utilisateur du 2026-09-15) : le tri et la densité de l'écran de recherche ont été retirés — la
justification upstream était fausse (mesuré : `display_option_picker.dart` porte des filtres d'affichage, pas un
tri ni une densité). Parité stricte retenue.
...
chemin ; la signature publique de l'`init` ne change pas pour les appelants existants.
Check post-impl: sh -c 'f=Sources/Features/Search/SearchViewModel.swift; grep -qE "var filter = SearchFilter\(\)" "$f" && grep -qE "var activeFilterCount" "$f" && grep -qE "var isFilterActive" "$f" && grep -qE "func applyFilters\(\) async" "$f" && grep -qE "func clearFilters\(\) async" "$f" && grep -qE "searchMode = \.metadata" "$f" && grep -qE "pendingExploreFilter = nil" "$f" && grep -qE "filter = SearchFilter\(\)" "$f" && echo PASS || echo FAIL'
...
Note (décision utilisateur du 2026-09-15) : le tri et la densité de l'écran de recherche ont été retirés — la justification upstream était fausse (mesuré : `display_option_picker.dart` porte des filtres d'affichage, pas un tri ni une densité). Parité stricte retenue.
```
...
(`filter.apply(to: &dto)`) et les deux dates
...
câblage interne.
Check post-impl: sh -c 'v=Sources/Features/Search/SearchViewModel.swift; t=Tests/SearchFilterTests.swift; test -f "$t" && grep -qE "filter\.apply\(to: &dto\)" "$v" && grep -qE "dto\.takenAfter = " "$v" && grep -qE "dto\.takenBefore = " "$v" && grep -qE "string\(from: " "$v" && grep -qE "lastMetadataSearchDto" "$t" && echo PASS || echo FAIL'
...
`vm.filter.*` ou `vm.applyFilters/clearFilters`) qui déclare son PROPRE `NavigationStack`
...
`await vm.clearFilters()`, qui remet `filter` à `SearchFilter()`.
...
```
### AC-5137 [type: new - parité stricte : plus aucune option d'affichage]
Assertion: l'onglet Search n'a **aucune** option d'affichage —
`Sources/Features/Search/SearchDisplayOptionsStore.swift` n'existe plus, `SearchViewModel` n'expose ni `sort`
ni `density`, et `SearchView` garde ses colonnes **fixes** (3, la valeur que l'onglet livrait) au lieu de les
dériver d'un réglage.
Note (décision utilisateur du 2026-09-15) : le tri et la densité de l'écran de recherche ont été retirés — la justification upstream était fausse (mesuré : `display_option_picker.dart` porte des filtres d'affichage, pas un tri ni une densité). Parité stricte retenue.
Check post-impl: sh -c 'st=Sources/Features/Search/SearchDisplayOptionsStore.swift; view=Sources/Features/Search/SearchView.swift; v=Sources/Features/Search/SearchViewModel.swift; ok=0; if test ! -f "$st" && test -f "$view" && test -f "$v"; then ok=1; grep -qF "private let columns = Array(repeating: GridItem(.flexible(), spacing: PVSpacing.s2), count: 3)" "$view" || ok=0; grep -qE "vm\.density" "$view" && ok=0; grep -qE "var sort:|var density:" "$v" && ok=0; grep -qE "searchSortOrder|searchGridDensity" "$view" "$v" && ok=0; fi; test "$ok" = 1 && echo PASS || echo FAIL'
Pre-state attendu: FAIL (`SearchDisplayOptionsStore.swift` existe et `Sources/Features/Search/SearchView.swift:30` dérive ses colonnes de `vm.density.columnCount` — l'assertion « le store n'existe plus » échoue donc, vérifié le 2026-09-15)
Post-state attendu: PASS
...
cas `@MainActor`) expose ≥ 7 cas couvrant l'écriture sélective du filtre, le
comptage, la borne de note, la bascule CLIP → Metadata, les deux effacements et la sérialisation des dates.
Note (décision utilisateur du 2026-09-15) : le tri et la densité de l'écran de recherche ont été retirés — la justification upstream était fausse (mesuré : `display_option_picker.dart` porte des filtres d'affichage, pas un tri ni une densité). Parité stricte retenue.
Check post-impl: sh -c 'f=Tests/SearchFilterTests.swift; ok=0; if test -f "$f"; then ok=1; n=$(grep -cE "func test_" "$f"); test "$n" -ge 7 || ok=0; for t in test_filterApply_writesOnlyNonNilFields test_activeCount_countsEachConstraintOnce test_rating_outOfRange_isIgnoredByApply test_ocrText_switchesFromSmartToMetadata test_clearFilters_resetsFilterButKeepsQuery test_clearSearch_doesNotClearFilters test_dispatch_writesDateRangeAsISO8601; do grep -qE "$t" "$f" || ok=0; done; fi; test "$ok" = 1 && echo PASS || echo FAIL'
Pre-state attendu: FAIL (fichier absent, vérifié le 2026-09-15)
Post-state attendu: PASS
