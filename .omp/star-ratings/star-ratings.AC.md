...
Check post-impl: sh -c 'a=Sources/Features/Search/SearchViewModel.swift; b=Sources/Features/Search/SearchView.swift; grep -q "var ratingFilter: Int?" "$a" && grep -q "func setRatingFilter(_ value: Int?) async" "$a" && grep -qF "get { filter.rating }" "$a" && grep -qF "dto.rating = rating" Sources/Core/Types/SearchFilter.swift && grep -qF "searchMode = .metadata" "$a" && grep -qF "await search()" "$a" && grep -q "private var ratingFilterMenu" "$b" && grep -qF "Any rating" "$b" && grep -q "Picker" "$b" && grep -qF "setRatingFilter" "$b" && m=$(grep -n "ratingFilterMenu" "$b" | cut -d: -f1 | head -1) && s=$(grep -n "searchModeMenu" "$b" | cut -d: -f1 | head -1) && test -n "$m" && test -n "$s" && test "$m" -lt "$s" && echo PASS || echo FAIL'
...
Note (arbitrage du 2026-09-15) : deux clauses retargées après la fusion de `search-filters` et de la migration de forme de requête. (1) `dto.rating = ratingFilter` vivait dans le ViewModel ; la projection appartient désormais à `SearchFilter.apply(to:)` (`dto.rating = rating`, propriété de la carte search-filters) et `ratingFilter` est devenu une **vue** sur `filter.rating` (`get { filter.rating }`) — une seule source d'état au lieu de deux. (2) la clause qui exigeait `clearSearch` → `ratingFilter = nil` a été retirée : sur ce point les deux cartes se contredisaient, et l'arbitrage va à `search-filters` (le filtre est un état de la feuille de filtres, qui a son propre Reset ; vider la requête ne vide pas les filtres — c'est ce que pin AC-5132, et le test a été renommé `test_clearSearch_keepsRatingFilter`). La matrice AC a montré la contradiction : AC-5067/AC-5068 étaient vertes en P3 et sont devenues rouges à la fusion de P4.
```
